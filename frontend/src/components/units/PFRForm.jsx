...
const defaultKinetics = () => ({ k_ref:0.01, T_ref_C:25, Ea_J_mol:0, n_order:1.0, delta_H_rxn:-50000, C_A0_mol_m3:1000, MW_A_g_mol:100 });
...
export default function PFRForm({ onResult }) {
  const [unitId, setUnitId]     = useState("R-201");
  ...
  const { mutate, isPending, isError, error } = useMutation({
    mutationFn: (payload) => client.post("/solve/pfr?include_log=true", payload).then(r => r.data),
  });

  function handleSolve() {
    const stream = { ...feed }; delete stream.fluid_id;
    mutate({ unit_id: unitId, feed_stream: stream, kinetics, operating, n_points: nPoints },
           { onSuccess: onResult });
  }
    ...
    <div className="space-y-4">
      <div className="card p-4 flex gap-4 items-end">
        <div>
          ...
          <input className="input w-32" value={unitId} onChange={e => setUnitId(e.target.value)} />
        </div>
        <div>
          <label className="label">Profile points</label>
          <input type="number" className="input w-24" value={nPoints}
            onChange={e => setNPoints(parseInt(e.target.value)||100)} step="10" min="10" max="500" />
        </div>
      </div>
